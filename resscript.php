...
<?php   
	...
	if ($title == "" || $st == "" || $en == "" || $date == "" || $court == "") {
		...
		if ($reserved) {
			echo "<p> The selected timeslot for this court is already reserved.  Going back to reserve again.</p>";
			echo '<meta http-equiv="refresh" content="5;url=http://umwtennis.org/reserve.php" />';
		...
		/*
		$isql2 = "SELECT res_id FROM tennis_reservations WHERE user_id='$userId' AND title='$title' AND start='$start' AND end='$end' AND date='$date' AND court='$court'";
		$iresult2 = @mysqli_query($conn, $isql2);
		$irow2 = @mysqli_fetch_array($iresult2, MYSQL_NUM);
		
		echo "your result $irow2";
		*/
		...
		//This JSON code was grabbed from http://stackoverflow.com/questions/7895335/append-data-to-a-json-file-with-php
                	$resQuery = "select MAX(res_id) from tennis_reservations;";
                	$iresQuery = @mysqli_query($conn,$resQuery);
                	$res_id = $iresQuery->fetch_row()[0];
                	$AdditionalArray = array("title" => "$title", "id" => "$date", "start" => "$start", "end" => "$end", "resourceId" => "$court", "resId" => "$res_id" );
